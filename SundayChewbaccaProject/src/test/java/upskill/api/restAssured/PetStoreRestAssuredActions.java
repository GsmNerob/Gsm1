...
public class PetStoreRestAssuredActions {
	...
	public void UpdateUsername(){
		...
		when().
			header("Content-Type", "application/json").									  //Setting the header
			put("https://petstore.swagger.io/v2/user/GsmNerob").							  //Setting the request url
		then().
			assertThat().statusCode(200).												  //Verify the status code
		and().
			body(matchesJsonSchema(new File(System.getProperty("user.dir") 
	...
	public void getUsername(){
		...
		when().
			header("Content-Type", "application/json").									  //Setting the header
			get("https://petstore.swagger.io/v2/user/GsmNerob").							  //Setting the request url
		then().
			assertThat().statusCode(200).												  //Verify the status code
		and().
			body(matchesJsonSchema(new File(System.getProperty("user.dir") 
	...
	public void deleteUsername(){
		...
		when().
			header("Content-Type", "application/json").								  //Setting the header
			delete("https://petstore.swagger.io/v2/user/GsmNerob").					  //Setting the request url
		then().
			assertThat().statusCode(200).												  //Verify the status code
		and().
			body(matchesJsonSchema(new File(System.getProperty("user.dir") 
